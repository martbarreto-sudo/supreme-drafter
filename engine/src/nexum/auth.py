...
não fiquem abertos à internet em deploy. NEXUM_TOKEN configurado via secret.
"""
...
def require_bearer(authorization: str | None = Header(default=None)) -> None:
    expected = os.getenv("NEXUM_TOKEN")
    if not expected:
        raise HTTPException(503, "NEXUM_TOKEN não configurado")
